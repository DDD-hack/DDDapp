...
@Observable
class DaemonDirectClient: NSObject {
    enum Status {
    ...
    private func sendLatestBPM() {
        guard let bpm = latestBPM, status == .connected, let task else { return }
        ...
        print("[DaemonDirect] ✅ sent bpm=\(Int(bpm))")
        task.send(.string(json)) { [weak self] error in
            guard let self, error != nil else { return }
            print("[DaemonDirect] send error: \(error!)")
    ...
    private func listenForClose() {
        task?.receive { [weak self] result in
            ...
            if case .failure(let error) = result {
                print("[DaemonDirect] connection lost: \(error)")
                Task { @MainActor in self.scheduleReconnect() }
            } else {
...
extension DaemonDirectClient: URLSessionWebSocketDelegate {
    nonisolated func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didOpenWithProtocol protocol: String?) {
        print("[DaemonDirect] connected ✅")
        Task { @MainActor in
    ...
    nonisolated func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didCloseWith closeCode: URLSessionWebSocketTask.CloseCode, reason: Data?) {
        print("[DaemonDirect] closed (code=\(closeCode.rawValue))")
        Task { @MainActor in self.scheduleReconnect() }
    }
}
